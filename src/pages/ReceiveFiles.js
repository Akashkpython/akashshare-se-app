  ...
  const checkBackendStatus = useCallback(async () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: 'The backend server is not reachable. Please ensure it is running on localhost:5005 or set REACT_APP_API_URL.'
      });
    }
  }, [addNotification]);
  ...
  const handleDownload = async () => {
    ...
    if (backendStatus !== 'online') {
      addNotification({
        ...
        message: 'Cannot download files. Ensure the backend is running on localhost:5005 (Electron/local) or your configured API URL.'
      });
      return;
    }
    ...
    try {
      ...
      if (error.message && error.message.includes('Cannot connect to server')) {
        errorMessage = 'Cannot connect to the backend server. Make sure it is running on localhost:5005 or your configured API URL.';
        ...
        <div className="mt-4 flex items-center justify-center">
          {backendStatus === 'checking' ? (
            ...
            <div className="flex items-center text-red-400">
              ...
              <span>Backend server is offline. Start it on localhost:5005 (Electron/local) or set REACT_APP_API_URL.</span>
            </div>
          )}
        </div>
      </motion.div>
