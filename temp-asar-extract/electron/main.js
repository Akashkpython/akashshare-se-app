...
async function ensureBackendEnv() {
  try {
    ...
    if (!fs.existsSync(envPath)) {
      log.warn('⚠️ Backend .env file not found, checking for alternatives...');
      
      // Try multiple options to create a working .env file
      const possibleEnvFiles = [
        rootEnvProductionPath,
...
async function createBackendProcess() {
  ...
  if (!fs.existsSync(backendPath)) {
    log.error(`❌ Backend script does not exist: ${backendPath}`);
    return null;
  }
  ...
  const backendEnv = {
    ...
    HOST: backendEnvCache.HOST || process.env.HOST || '0.0.0.0',
    MONGO_URI: backendEnvCache.MONGO_URI || process.env.MONGO_URI || 'mongodb+srv://dreamguy499:xyEz3A4YI5PkMwjR@akashshare.znzo9ht.mongodb.net/?retryWrites=true&w=majority&appName=akashshare',
    JWT_SECRET: backendEnvCache.JWT_SECRET || process.env.JWT_SECRET || 'f8e7d6c5b4a39281706f5e4d3c2b1a0987654321fedcba0987654321fedcba0987654321fedcba09',
    START_SERVER: 'true', // Ensure the backend server starts
  ...
  log.info('🔧 Backend environment variables:', {
    ...
    MONGO_URI: backendEnv.MONGO_URI ? 'set' : 'not set',
    JWT_SECRET: backendEnv.JWT_SECRET ? 'set' : 'not set'
  });
...
function createTray() {
  ...
  const iconPaths = [
    ...
    path.join(process.resourcesPath, 'app.asar.unpacked', 'build-resources', 'icon.ico'),
    path.join(process.resourcesPath, 'build-resources', 'icon.ico'),
    path.join(process.resourcesPath, 'app.asar.unpacked', 'public', 'Akashshareicon.png'),
    path.join(process.resourcesPath, 'public', 'Akashshareicon.png'),
    path.join(process.resourcesPath, 'build', 'Akashshareicon.png')
  ];
  ...
  if (mainWindow) {
    try {
      ...
      if (mainWindow.isMaximized()) {
        ...
        log.info(`⏱️ Window unmaximized in ${Date.now() - maximizeStartTime}ms`);
        return { success: true, maximized: false };
      } else {
        mainWindow.maximize();
        log.info(`⏱️ Window maximized in ${Date.now() - maximizeStartTime}ms`);
        return { success: true, maximized: true };
      }
    } catch (error) {
      log.error('❌ Error (un)maximizing window:', error);
      return { success: false, error: error.message };
    }
  } else {
    log.warn('⚠️ No main window to (un)maximize');
    return { success: false, error: 'No main window' };
  }
});
  ...
  if (mainWindow) {
    try {
      ...
      if (app.isPackaged) {
        ...
        log.info(`⏱️ Window hidden in ${Date.now() - closeStartTime}ms`);
        return { success: true, action: 'hidden' };
      } else {
        mainWindow.close();
        log.info(`⏱️ Window closed in ${Date.now() - closeStartTime}ms`);
        return { success: true, action: 'closed' };
      }
    } catch (error) {
      log.error('❌ Error closing/hiding window:', error);
      return { success: false, error: error.message };
    }
  } else {
    log.warn('⚠️ No main window to close/hide');
    return { success: false, error: 'No main window' };
  }
});
